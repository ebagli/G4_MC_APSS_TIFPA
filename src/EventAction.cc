...
void EventAction::EndOfEventAction(const G4Event* evt){
    ...
    if(bTotalHits > 3){
        ...
        analysisManager->FillNtupleDColumn(0, angXin  / CLHEP::mrad);
        analysisManager->FillNtupleDColumn(1, angYin  / CLHEP::mrad);
        
        analysisManager->FillNtupleDColumn(2, ssd[0].x()  / CLHEP::mm);
        analysisManager->FillNtupleDColumn(3, ssd[0].y()  / CLHEP::mm);
        
        if(bTotalHits == 4){
            G4double angXout = (ssd[3].x() - ssd[2].x()) / (ssd[3].z() - ssd[2].z());
            G4double angYout = (ssd[3].y() - ssd[2].y()) / (ssd[3].z() - ssd[2].z());
            analysisManager->FillNtupleDColumn(4, angXout / CLHEP::mrad);
            analysisManager->FillNtupleDColumn(5, angYout / CLHEP::mrad);
        }
        else{
        ...
        analysisManager->FillNtupleDColumn(6, ssd[2].x() / CLHEP::mm);
        analysisManager->FillNtupleDColumn(7, ssd[2].y() / CLHEP::mm);
